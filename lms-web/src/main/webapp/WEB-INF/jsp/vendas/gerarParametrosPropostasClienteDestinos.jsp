...
<adsm:window service="lms.vendas.gerarParametrosPropostaAction">
	...
	<adsm:grid
		...
		autoSearch="false"
		service="lms.vendas.gerarParametrosPropostaAction.generateDestinosProposta"
		...
		<adsm:buttonBar>
			...
			<adsm:button caption="salvar" id="btnSalvar" onclick="storeDestinosProposta();" disabled="true"/>
...
<script language="javascript" type="text/javascript">
	...
	function storeDestinosProposta() {
		...
		var mapIds = destinoPropostaGridDef.getSelectedIds();
	   	if (mapIds.ids.length > 0 || hasValue(getElementValue("idProposta"))) {
	   		if (validateStore()) {
				...
				var sdo = createServiceDataObject("lms.vendas.gerarParametrosPropostaAction.storeDestinosProposta", "storeDestinosProposta", data);
				xmit({serviceDataObjects:[sdo]});
			}
		}
	}
	function storeDestinosProposta_cb(data, error) {
		if(error != undefined) {
			alert(error);
			return;
		}

		getTabGroup(this.document).getTab("cad").getElementById("tipoLocalizacaoMunicipioByIdTipoLocalizacaoOrigem.idTipoLocalizacaoMunicipio").disabled = true;
	...
	function disableRows(rowIndex, disabled) {
	    ...
	    if(isTabela(getElementValue("tpIndicadorAdvalorem"))) {
	    	setDisabled(destinoPropostaGridDef.getCellObject(rowIndex,"vlAdvalorem"), true);
	    } else {
	    	setDisabled(destinoPropostaGridDef.getCellObject(rowIndex,"vlAdvalorem"), disabled);
	    }

...
<adsm:window service="lms.municipios.manterRotasViagemAction" >
	...
	<adsm:form action="/municipios/manterTrechosRotaViagem" idProperty="idTrechoRotaIdaVolta"
			service="lms.municipios.manterRotasViagemAction.findByIdTrechoRotaVolta" onDataLoadCallBack="trechoLoad" >
		<adsm:masterLink idProperty="idRotaViagem" showSaveAll="true">
			<adsm:masterLinkItem label="rota" property="rotaVolta.dsRota" itemWidth="100"/>
			<adsm:hidden property="idRotaIdaVolta" />
			<adsm:hidden property="tipoRotaIdaVolta" value="V" />
		</adsm:masterLink>
		...
		<adsm:textbox dataType="JTTime" property="nrTempoViagem" mask="hhh:mm"
				label="tempoViagem" size="6" required="true" maxLength="6" labelWidth="17%" unit="h" width="33%"/>
		<adsm:textbox dataType="JTTime" property="nrTempoOperacao" mask="hhh:mm"
				label="tempoOperacao" size="6" required="true" maxLength="6" labelWidth="17%" unit="h" width="33%"/>
		...
		<adsm:buttonBar freeLayout="true" >
			...
			<adsm:storeButton caption="incluirTrecho" callbackProperty="afterStore"
					service="lms.municipios.manterRotasViagemAction.saveTrechoRotaVolta" />
			<adsm:newButton />
		</adsm:buttonBar>
	</adsm:form>

	<adsm:grid idProperty="idTrechoRotaIdaVolta" property="trechoRotaIdaVolta" detailFrameName="volta"
				rows="6" unique="true" autoSearch="false" scrollBars="horizontal" gridHeight="120"
				service="lms.municipios.manterRotasViagemAction.findPaginatedTrechoRotaVolta"
				rowCountService="lms.municipios.manterRotasViagemAction.getRowCountTrechoRotaVolta" 
		...
		<adsm:gridColumn title="tempoViagem" property="nrTempoViagem" unit="h" dataType="text" width="90" align="center"/>
		<adsm:gridColumn title="tempoOperacao" property="nrTempoOperacao" unit="h" dataType="text" width="90" align="center" />
	...
	<adsm:buttonBar>
		<adsm:removeButton caption="excluirTrecho" service="lms.municipios.manterRotasViagemAction.removeByIdsTrechoRotaVolta" /> 
	</adsm:buttonBar>
	</adsm:grid>
</adsm:window> 
<script language="javascript" type="text/javascript">
	...
	function tabShowRotas() {
		...
		var list = tabDet.tabOwnerFrame.document.getElementById("rotaVolta.filiaisRota");		
		...
		setElementValue("rotaIdaVolta.nrDistancia",tabDet.tabOwnerFrame.document.getElementById("rotaVolta.nrDistancia"));
		setElementValue("idRotaIdaVolta",tabDet.tabOwnerFrame.document.getElementById("rotaVolta.idRotaIdaVolta"));
		setElementValue("dsRotaToLink",tabDet.tabOwnerFrame.document.getElementById("rotaVolta.dsRota"));
		setElementValue("nrRotaToLink",tabDet.tabOwnerFrame.document.getElementById("rotaVolta.nrRota"));
	...
	/*
	 * Copia dados da listBox rota Volta para combo.
	 */
	...
	function filiaisChange(filialHidden) {
		...
		if (origem.selectedIndex == 1 && destino.selectedIndex == destino.length-1) {
			var tabGroup = getTabGroup(this.document);
			var tabDet = tabGroup.getTab("cad");
			var distancia = tabDet.tabOwnerFrame.document.getElementById("rotaVolta.nrDistancia");
	...
	function afterStore_cb(data,error,key) {	
		...
		if (resultConfirm)
			storeButtonScript('lms.municipios.manterRotasViagemAction.saveTrechoAfterValidationConfirmedVolta',
					'storeItem', document.getElementById('form_idTrechoRotaIdaVolta'));
		else if (!resultConfirm)
			storeButtonScript('lms.municipios.manterRotasViagemAction.saveTrechoAfterValidationRefusedVolta',
					'storeItem', document.getElementById('form_idTrechoRotaIdaVolta'));
	}
	...
	function atualizaVlFreteCarreteiro() {
		var data = new Array();
		var sdo = createServiceDataObject("lms.municipios.manterRotasViagemAction.calculaTotalRateioVolta","atualizaVlFreteCarreteiro",data);
		xmit({serviceDataObjects:[sdo]});
	}
	...
	function atualizaVlFreteCarreteiro_cb(data,error) {
		...
		if (data != undefined) {
			...
			if (vlFreteCarreteiro != undefined && vlFreteCarreteiro != ""){
				tabCad.getElementById("rotaVolta.vlFreteCarreteiro").value = vlFreteCarreteiro;
			} else {
				tabCad.getElementById("rotaVolta.vlFreteCarreteiro").value = "";
			}
			telaCad.calcFreteKm("Volta");
		}
	}

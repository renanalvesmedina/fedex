...
<script language="javascript" type="text/javascript">
	function carregaDados() {
		...
		var filter = [];
		setNestedBeanPropertyValue(filter, "idGeneralidade", idTabelaPrecoParcela);
		var sdo = createServiceDataObject("lms.tabelaprecos.generalidadeService.find", "carregaDados", filter);
	...
	function carregaDados_cb(data, error) {
		...
		var idGeneralidade = getNestedBeanPropertyValue(data,":0.idGeneralidade");
		/*se já existe preço para a parcela
		atribui os valores nos campos (para edição)*/
		if(idGeneralidade > 0) {
			setElementValue("idGeneralidade", idGeneralidade);
			var obj = document.getElementById("vlGeneralidade");
			setElementValue(obj, setFormat(obj, getNestedBeanPropertyValue(data, ":0.vlGeneralidade")));
			obj = document.getElementById("vlMinimo");
			setElementValue(obj, setFormat(obj, getNestedBeanPropertyValue(data, ":0.vlMinimo")));
			obj = document.getElementById("psMinimo");
			setElementValue(obj, setFormat(obj, getNestedBeanPropertyValue(data, ":0.psMinimo")));
		}
	}
...
<adsm:window
	service="lms.tabelaprecos.precificarParcelaGeneralidadeAction"
	onPageLoad="carregaDados">
	<adsm:form
		action="/tabelaPrecos/precificarParcelaGeneralidade"
		...
		<adsm:buttonBar>
			<adsm:storeButton
				id="btnSalvar"
				service="lms.tabelaprecos.precificarParcelaGeneralidadeAction.storeAtualizaTabela"

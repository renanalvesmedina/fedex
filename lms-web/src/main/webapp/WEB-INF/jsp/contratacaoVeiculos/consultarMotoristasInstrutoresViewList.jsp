...
//LMS 7128	
function codFuncaoOnChange(){
	if (getElementValue("codFuncao.cargo.codigo") === '025'){
	
		resetValue('codFuncao.codigo');
		resetValue('codFuncao.nome');
		setElementValue("codFuncaoInstMotorista",'025.0002,025.04');
		setDisabled("codFuncao.codigo", true);
	
	}else{
	
		resetValue('codFuncao.codigo');
		resetValue('codFuncao.nome');
		setDisabled("codFuncao.codigo", false);
	}
...
<adsm:window service="lms.configuracoes.FuncionarioService" onPageLoadCallBack="pageLoadCustom" onPageLoad="pageLoadFuncionario">
	<adsm:form action="/contratacaoVeiculos/consultarMotoristasInstrutoresView">
		...
		<adsm:combobox property="codFuncao.cargo.codigo" label="cargo" labelWidth="15%" width="35%" 
						service="lms.contratacaoveiculos.manterMotoristasAction.findCargoMotoristaInstrutor" 
						optionLabelProperty="nome" optionProperty="codigo" boxWidth="240" disabled="false" onchange = "codFuncaoOnChange();" />
		...
		<adsm:grid idProperty="idUsuario" property="usuario" gridHeight="200" unique="true" rows="12"
				   service="lms.configuracoes.manterUsuarioFuncionarioAction.findPaginatedInstrutores" 
		       	   rowCountService="lms.configuracoes.manterUsuarioFuncionarioAction.getRowCountInstrutores">

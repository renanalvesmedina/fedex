...
<adsm:window service="lms.expedicao.digitarAWBCiasAereasAction">
	<adsm:form
		id="filter.form"
		action="/expedicao/digitarAWBCiasAereas"
		service="lms.expedicao.digitarAWBCiasAereasAction">
		...
		<adsm:lookup
			...
			service="lms.expedicao.digitarAWBCiasAereasAction.findLookupFilial"
			dataType="text"
			property="filial"
			idProperty="idFilial"
		...
		<adsm:lookup
			...
			service="lms.expedicao.digitarAWBCiasAereasAction.findLookupMunicipio"
			property="municipio"
		...
		<adsm:lookup 
   		...
   		<adsm:combobox label="numeroAWB" property="tpStatusAwb"
					   ...
					   defaultValue="P" disabled="true" renderOptions="true">
			...
			<adsm:lookup property="ciaFilialMercurio.empresa" 
						 ...
						 service="lms.expedicao.digitarAWBCiasAereasAction.findLookupSgCiaAerea"
						 action="" 	
						 size="3" maxLength="3"						 
					 	 picker="false">
			</adsm:lookup>
	        ...
	        <adsm:lookup dataType="integer" size="13" maxLength="11" 
	        	...
	        	criteriaSerializable="true"
	 			service="lms.expedicao.digitarAWBCiasAereasAction.findLookupAwb"
	...
	<adsm:grid
		...
		property="ctrcList"
		service="lms.expedicao.digitarAWBCiasAereasAction.findCtrc"
		showPagging="false"
		gridHeight="120"
		autoSearch="false"
		unique="false"
...
function clickGerarSugestao() {
	...
	var idPreAwb = getElementValue("awb.idAwb");
	...
	var blPreAwb = (idPreAwb == null || idPreAwb == "");
	...
	if(blAeroporto && blFilial && blMunicipio && blManifestoViagem && blPreAwb) {
...
function clickConsolidarCarga() {
	...
	if(!execute) {
		...
		var service = "lms.expedicao.digitarAWBCiasAereasCTRCsAction.consolidarCarga";
...
function consolidarCarga_cb(data, error) {
	if(error) {
		...
		var frame = parent.document.frames["dadosGerais_iframe"];
...
function resetManifestoViagem() {
	...
	function findDadosAwb(idAwb, idConhecimento){
		if( (idAwb != null && idAwb != "") || (idConhecimento != null && idConhecimento != "") ) {
		    var sdo = createServiceDataObject("lms.coleta.cadastrarPedidoColetaAction.findDadosColetaAwb", "findDadosAwb", 
		    		{idAwb:idAwb, idConhecimento:idConhecimento}
		    );
		    if( idAwb != null && idAwb != "" ) {
		    	xmit({serviceDataObjects:[sdo]});
		    }else{
		    	xmit({serviceDataObjects:[sdo], onXmitDone:"findDadosAwbDoctoServico"});
		    }
		}else{
			limparDadosAwb();
		}
...
function findAwb_cb(data, error){
	if(data != null){
		...
		setElementValue("awb.idAwb", data.idAwb);
		setElementValue("awb.nrAwb", data.idAwb);			
...
function findDadosAwb(idAwb){
	if(idAwb != null && idAwb != "") {
	    var sdo = createServiceDataObject("lms.expedicao.digitarAWBCiasAereasAction.findDadosColetaAwb", "findDadosAwb", 
	    		{idAwb:idAwb}
	    );
    	xmit({serviceDataObjects:[sdo]});
	}else{
...
function findDadosAwb_cb(data, error){
	if (error){
		...
		if(getElementValue("awb.idAwb")) {
			setFocus("awb.idAwb");
			alert('LMS-00061 - ' + getI18nMessage('LMS-00061'));				
			resetValue("awb.idAwb");
		}
	}
}

function limparDadosAwb(){
	resetValue("awb.idAwb");
	resetValue("awb.nrAwb");
	resetValue("tpStatusAwb");

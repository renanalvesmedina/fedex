 ...
 * @spring.bean id="lms.contasreceber.emitirBoletosAction"
 */

public class EmitirBoletosAction extends EmitirDactesFaturaBoletoAction {

	// FIXME remover esta dependencia após o refactoring no método execute.
	...
	public File execute(TypedFlatMap tfm) throws Exception {
		...
		if(tfm.getLong("cedente.idCedente") != null){
			...
			/** Seta o nome do report */
			reportFile = super.execute(tfm);
			
		}else{
			
			// TODO Mover esta implementação para uma service
			validateIntervalBoletos(tfm);
			
			/** Objeto que agrupa n relatórios em um único */
			// FIXME definir o nome correto do relatório a partir de uma mensagem i18n
			...
			/** Seta o nome do report */
			// FIXME ao ser feito o refactoing para uma Service não é necessário
			// ter esta chamada a propria service de relatório possui métodos apropriados
			// para execução de MultiReports.
	...
	public File executeEmissaoCTE(TypedFlatMap typedFlatMap){
		...
		return emiteCTEbyListFaturas(idsFaturas);
	}
	...
	public String executeEmitirCTEWithLocator(TypedFlatMap parameters){
		try {
			File file = executeEmissaoCTE(parameters);
			if (file != null) {
				String locator = reportExecutionManager.generateReportLocator(file); 
	...
	public File executeEmissaoNTE(TypedFlatMap typedFlatMap){
		EmitirBoletoService emitirBoletoService = (EmitirBoletoService)reportServiceSupport;
		List<Long> idsFaturas = emitirBoletoService.findFaturasBoletoByCriteria(typedFlatMap);
		
		List<Map<String, Object>> listCte = monitoramentoDocEletronicoService.findByListFatura(idsFaturas, ConstantesExpedicao.NOTA_FISCAL_TRANSPORTE_ELETRONICA);
		return emiteNTE(listCte);
	}

	public String executeEmitirNTEWithLocator(TypedFlatMap parameters){
		try {
			File file = executeEmissaoNTE(parameters);
			if (file != null) {
				String locator = reportExecutionManager.generateReportLocator(file); 
	...
	public File executeEmissaoNSE(TypedFlatMap typedFlatMap){
		EmitirBoletoService emitirBoletoService = (EmitirBoletoService)reportServiceSupport;
		List<Long> idsFaturas = emitirBoletoService.findFaturasBoletoByCriteria(typedFlatMap);
		
		List<Map<String, Object>> listCte = monitoramentoDocEletronicoService.findByListFatura(idsFaturas, ConstantesExpedicao.NOTA_FISCAL_SERVICO_ELETRONICA);
		return emiteNSE(listCte);
	}

	public String executeEmitirNSEWithLocator(TypedFlatMap parameters){
		try {
			File file = executeEmissaoNSE(parameters);
			if (file != null) {
				String locator = reportExecutionManager.generateReportLocator(file); 

 ...
 * 
 * @spring.bean id="lms.entrega.emitirEficienciaEntregaDataService"
 * @spring.property name="reportName" value="com/mercurio/lms/entrega/report/emitirEficienciaEntregaData.jasper"
 */
public class EmitirEficienciaEntregaDataService extends ReportServiceSupport {
	...
	private SqlTemplate getSqlTemplate(TypedFlatMap parameters) {	
		...
		mainSql.addOrderBy("DADOS_01.DT_EVENTO"); 
	...
	private String getSqlDados(SqlTemplate mainSql,TypedFlatMap parameters) {
		...
		sql.addProjection(new StringBuilder()
				.append("Max(TO_SECONDS(")
				.append("CAST (Trunc(DADOS_CC.DH_CHEGADA_COLETA_ENTREGA,'Mi') AS TIMESTAMP)")
				.append(" - ")
				.append("CAST (Trunc(DADOS_CC.DH_SAIDA_COLETA_ENTREGA,'Mi') AS TIMESTAMP)")
				.append(")) as TEMPO_ENTREGA")
				.toString());
		sql.addProjection("Trunc(ME.DH_FECHAMENTO)","DT_EVENTO");
		sql.addProjection("Max(DADOS_CC.ID_NOTA_CREDITO)","ID_NOTA_CREDITO");

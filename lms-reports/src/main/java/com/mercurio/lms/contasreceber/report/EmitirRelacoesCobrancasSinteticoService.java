 ...
 * @spring.bean id="lms.contasreceber.emitirRelacoesCobrancasSinteticoService"
 * @spring.property name="reportName" value="com/mercurio/lms/contasreceber/report/emitirRelacoesCobrancasSintetico.jasper" 
 */

public class EmitirRelacoesCobrancasSinteticoService extends ReportServiceSupport {
	...
	private SqlTemplate getSql(TypedFlatMap tfm) {
		...
		boolean listarDocumentosServicos = tfm.getBoolean("listarDocumentosServicos").booleanValue();
		...
		if (listarDocumentosServicos) {
			sql.addProjection("ds.tp_documento_servico", "TIPO_DOCUMENTO");
			sql.addProjection("fd.sg_filial || ' - ' || ds.nr_docto_servico", "DOCUMENTO_SERVICO");
			sql.addProjection("ddsf.vl_devido", "VALOR_DOCUMENTO");
		} else {
			sql.addProjection("'FAT'", "TIPO_DOCUMENTO");
			sql.addProjection("ff.sg_filial || ' - ' || fat.nr_fatura", "DOCUMENTO_SERVICO");
		...
		sql.addCriteria("rc.nr_relacao_cobranca_filial", "=", tfm.getLong("nrRelacaoCobranca"));
		...
		sql.addFilterSummary("numeroRelacao", nrRelacaoCobranca);

...
public class NotaCreditoCalculoUmReportRunner extends ExcelRemoteReportsRunner {

	private static final String REPORT_NAME = "notaCreditoCalculoUm";
	...
	public NotaCreditoCalculoUmReportRunner(String reportHostUrl) {
		super(reportHostUrl);
	}	
	...
	private void populateDadosNotaCredito(){
		...
		ExcelReportUtils.setStringVar(sheet, "dsModeloMeioTransporte", dadosNotaCredito.get("DS_MARCA_MEIO_TRANSPORTE").toString());
		ExcelReportUtils.setStringVar(sheet, "dsMarcaMeioTransporte", dadosNotaCredito.get("DS_MODELO_MEIO_TRANSPORTE").toString());
		ExcelReportUtils.setStringVar(sheet, "dsRota", dadosNotaCredito.get("DS_ROTA").toString());
		ExcelReportUtils.setNumericVar(sheet, "vlFranquia", Double.valueOf(dadosNotaCredito.get("NR_FRANQUIA_KM").toString()));
	...
	private void populateTabelaDeFrete(){
		...
		for (Map tabelaFrete : listTabelaFrete){
			...
			ExcelReportUtils.setNumericVar(sheet, "vlFracaoPeso", Double.valueOf(tabelaFrete.get("vlFracaoPeso").toString()));			
	...
	private void populateTabelaManifestoEntrega() {	
		...
		for (String nrManifestoEntrega: mapTabelaManifestoEntrega.keySet()){
			...
			if (listEntrega.size() > 1){
				for (int i = 0; i < listEntrega.size() - 1; i++){
					ExcelReportUtils.duplicateRow(sheet, row.getRowNum(), row.getRowNum() + 1 + i);
				}
			}
			for (Map doctoServico : listEntrega){		
				ExcelReportUtils.setStringVar(sheet, "sgFilialDoctoServico", checkValue(doctoServico.get("sgFilial")));
				ExcelReportUtils.setNumericVar(sheet, "nrDoctoServico", Double.valueOf(checkValue(doctoServico.get("nrDoctoServico"))));
				ExcelReportUtils.setStringVar(sheet, "enderecoReal", checkValue(doctoServico.get("enderecoReal")));
				ExcelReportUtils.setNumericVar(sheet, "vlPesoEntrega", Double.valueOf(checkValue(doctoServico.get("vlPeso"))));
				ExcelReportUtils.setNumericVar(sheet, "qtEventoEntrega", Double.valueOf(checkValue(doctoServico.get("qtEvento"))));
				ExcelReportUtils.setNumericVar(sheet, "qtVolumeEntrega", Double.valueOf(checkValue(doctoServico.get("qtVolume"))));		
			}
		}
	}
	

	//Test if the object is null so returns the prover value
	private static String checkValue(Object object) {
		if (object != null) {
	...
	private void populateTotalParcelas(){
		...
		ExcelReportUtils.setNumericVar(sheet, "vlTotalFracaoPeso", Double.valueOf(dadosTotalParcelas.get("vlTotalFracaoPeso").toString()));			

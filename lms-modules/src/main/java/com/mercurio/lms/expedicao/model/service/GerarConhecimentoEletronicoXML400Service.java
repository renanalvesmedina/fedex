...
import com.mercurio.lms.cte.model.v400.*;
import com.mercurio.lms.cte.model.v400.types.*;
...
public class GerarConhecimentoEletronicoXML400Service {
    ...
    public void gerarICMSUFFim(Imp imp, DoctoServico doctoServico, Conhecimento cto) {
        ...
        if (doctoServico.getVlImpostoDifal() != null) {
            icmsufFim = getICMSUFFimValoresPadrao(cto);
            UnidadeFederativa ufFim = getUFFim(cto);

            if("S".equalsIgnoreCase(geraICMSUfFimCte)) {
                executePreenchimentoTagsICMSUFFim(cto, doctoServico, icmsufFim, ufFim.getIdUnidadeFederativa());
            }
    ...
    private void executePreenchimentoTagsICMSUFFim(Conhecimento cto, DoctoServico doctoServico, ICMSUFFim icmsufFim, Long idUFFim) {
        ...
        if (BooleanUtils.isTrue(hasTTINaoContribuinte) && aii.getPcAliquota().compareTo(cto.getPcAliquotaIcms()) >= 0) {
            ...
            if ((BigDecimalUtils.hasValue(cto.getVlImposto()) || BigDecimalUtils.hasValue(cto.getVlIcmsSubstituicaoTributaria())) && !idUfFilialOrigem.equals(idUFFim)
                ...
                icmsufFim.setPICMSUFFim(formatDoisDecimais.format(doctoServico.getPcIcmsUfFim()));
                icmsufFim.setPICMSInter(formatDoisDecimais.format(cto.getPcAliquotaIcms()));
                BigDecimal vICMSUFFim = doctoServico.getVlImpostoDifal();
                icmsufFim.setVICMSUFFim(formatDoisDecimais.format(vICMSUFFim));
                BigDecimal vICMSUFIni = BigDecimal.ZERO;
    ...
    private void gerarInfCte(final CTe cte, final Conhecimento conhecimento, final MonitoramentoDocEletronico monitoramentoDocEletronico, Contingencia emContingencia) throws ValidationException {
        ...
        gerarImp(infCte, doctoServico, conhecimento, valorTotalTributo);
...
//			TODO Verificar com analista se deve preencher ja existe na versão 1.04 mas não era preenchido
//			receb.setEmail(conhecimento.getClienteByIdClienteConsignatario().getPessoa().getDsEmail());
    ...
    private void gerarImp(final InfCte infCte, final DoctoServico doctoServico, final Conhecimento conhecimento, String valorTotalTributo) {
        Imp imp = new Imp();
        imp.setVTotTrib(valorTotalTributo);
        gerarICMS(imp, conhecimento);
        gerarICMSUFFim(imp, doctoServico, conhecimento);
        infCte.setImp(imp);
    }
...
//			TODO: LMSA-798
    ...
    private void gerarPass(final Fluxo fluxo, final Conhecimento conhecimento) {
        //desconsidera a inicial e a final

...
@Assynchronous
public class EnviarCobrancaFaturasEmailService extends CrudService<Fatura, Long> {
	...
	@AssynchronousMethod(name = "contasreceber.enviarCobrancaFaturasEmail", type = BatchType.BATCH_SERVICE, feedback = BatchFeedbackType.ON_ERROR)
	...
	public void executeEnviarFaturasEmail() {
		List<Object[]> faturas = faturaDao.findCobrancaFaturasNaoEnviadasEmail();
		...
		for (int i = 0; i < faturas.size(); i++) {
			...
			if (fatura.getIdCliente().equals(lastClientId) ){ // cliente novo
				...
				if ( !clientesComEmail.isEmpty() ){
					storeGeraEmailComunicacao(clientesComEmail.remove(lastClientId),"CO");
				}
				agrupaFaturas(clientesComEmail, clientesSemEmail, fatura);
			}
			if ( !lastRegionalId.equals(fatura.getIdRegional()) ){
				storeGeraEmailComunicacao(clientesSemEmail.remove(lastRegionalId),"SC");
			}
			lastClientId = fatura.getIdCliente();
			lastRegionalId = fatura.getIdRegional();
		}
		if ( !clientesSemEmail.isEmpty() ){
			...
			while(keys.hasNext()){
				storeGeraEmailComunicacao(clientesSemEmail.remove(keys.next()),"SC");
			}
		}
		if ( !clientesComEmail.isEmpty() ){
			Iterator<Long> keys = clientesComEmail.keySet().iterator();
			while(keys.hasNext()){
				storeGeraEmailComunicacao(clientesComEmail.remove(keys.next()),"CO");
			}
		}
	}
